...
void setup() {
    ...
    Serial.print("Free PSRAM after framebuffer init: ");
...
void loop() {
    ...
    uint16_t touch_x = 0;
    uint16_t touch_y = 0;
    const bool is_currently_touched = touch.getTouch(touch_x, touch_y);
    ...
    if (is_currently_touched) {
        ...
        if (sync_bar_x <= 38 ||
            sync_bar_x >= (Config::SCREEN_WIDTH - BAR_WIDTH - 38)) {
        ...
        if (box_x <= 38 ||
            box_x >= (Config::SCREEN_WIDTH - BOX_SIZE - 38)) {
            box_dx = -box_dx;
        }

        if (box_y <= 38 ||
            box_y >= (Config::SCREEN_HEIGHT - BOX_SIZE - 38)) {
            box_dy = -box_dy;
        }
    }

    if (!canvas.swap()) {
        Serial.println("Framebuffer present failed.");
    }
}
